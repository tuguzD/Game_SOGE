...
{
    /*
     This is not autistic concept, it's just special, I promise.... Okay, maybe it is autistic, but let me explain something.
     ...
     Then, instead of having some logical function to decide what's the realization we want to use, we just have compile time macros
     which do it for us. Even so, we still need the implementation class name to operate with it, that's why we typedef implementation
     class at the end of the file to some standardized naming structure for this.

     For example the SDLKeyboard has the **typedef SDLKeyboard ImplKeyboard** thing.

     But we are still using Key types in the Keyboard implementation. So, when we are trying to make some friends with ImplKeyboard,
     compile count this as overriding of ImplKeyboard class and throws an error. That's why this weird thing has to be.
     ...
     Example of usage:
        ...
        public:
            std::string access(Target& target, int value) {
                ...
                Accessor<Target, Target, std::string, int> accessor(Target::getFriendPtr());
    ...
    */
    ...
    public:
        // TODO: Figure out somehow to keep it protected. This us fucking dangerous right now
